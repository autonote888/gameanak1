...
def muat_aset(nama_file, ukuran):
    ...
    if os.path.exists(path_lengkap):
        try:
            ...
            # Menghapus warning iCCP profil warna agar lancar di server
...
# Menggunakan None agar tidak mencari font sistem yang hilang di server
...
def buat_biota():
    ...
    y = random.randint(permukaan_air + 80, 780)
...
while True:
    ...
    if not game_pause:
        ...
        for b in biota_list:
            ...
            if b["rect"].x > LEBAR + 350 or b["rect"].x < -350:
                b["rect"].x = -250 if b["arah"] == 1 else LEBAR + 200
    ...
    # UI Papan Skor (3 Kolom x 4 Baris)
