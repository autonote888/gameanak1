...
pygame.init()
# Mencari folder utama tempat file game2.py berada
...
def muat_aset(nama_file, ukuran):
    ...
    if os.path.exists(path_lengkap):
        try:
            ...
            # Menghilangkan error iCCP dengan convert()
...
# Gunakan Font Default agar tidak error di server
...
for _ in range(10): gelembung_list.append(buat_gelembung())
...
while True:
    ...
    if not game_pause:
        ...
        for g in gelembung_list:
            g["y"] -= g["speed"]
            if g["y"] < permukaan_air: g["y"] = TINGGI; g["x"] = random.randint(0, LEBAR)
    ...
    for i, (jenis, jml) in enumerate(list(statistik.items())[:8]):
        if ASET[jenis]: layar.blit(pygame.transform.scale(ASET[jenis], (35, 25)), (sx, sy))
        txt = font_skor.render(f": {jml}", True, (0, 0, 0))
        layar.blit(txt, (sx + 45, sy - 2))
        sy += 35
        if (i+1) % 4 == 0: sx += 140; sy = 25

    # Tombol STOP & Overlay Pause
